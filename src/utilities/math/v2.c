...
inline V2 V2_SCALE(V2 *a, float b)
{
...
inline float V2_INT_DOT(V2_INT *a, V2_INT *b)
{
    return a->x * b->x + a->y * b->y;
}

inline V2 V2_INT_SUB(V2_INT *a, V2_INT *b)
{
    return (V2){(float)(a->x - b->x), (float)(a->y - b->y)};
}

inline float V2_MAGNITUDE(V2 *a)
{
...
inline float V2_INT_MAGNITUDE(V2_INT *a)
{
    return sqrtf(a->x * a->x + a->y * a->y);
}